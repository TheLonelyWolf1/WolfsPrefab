...
public class PortalSwordCommand implements CommandExecutor {
    org.bukkit.plugin.Plugin Plugin = Wolfsprefab.getInstance();

    public boolean onCommand(@NotNull CommandSender sender, Command cmd, @NotNull String label, String[] args) {
        if(cmd.getName().equalsIgnoreCase("wpportalsword")) {
            if(sender instanceof Player) {
                Player p = (Player)sender;
                if(p.hasPermission("wolfprefab.admin.give")) {
                    if(args.length == 0) {
                        GiveFunctions.givePortalSword(p, 64);
                    }else if(args.length == 1){
                        String amount = args[0];
                        GiveFunctions.givePortalSword(p, Integer.parseInt(amount));
                    }else if(args.length == 2){
                        Player other_player = Bukkit.getPlayer(args[0]);
                        assert other_player != null;
                        String amount = args[1];
                        GiveFunctions.givePortalSword(other_player, Integer.parseInt(amount));
                    }else{
